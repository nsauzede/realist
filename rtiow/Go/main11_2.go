...
func (c Camera) get_ray(s float32, t float32) Ray {
	return Ray{c.origin,
		vsub(vadd(c.lower_left_corner, vadd(vmul(s,c.horizontal), vmul(t,c.vertical))), c.origin)}
}
...
func make_camera(lookfrom Vec3, lookat Vec3, vup Vec3, vfov float32, aspect float32) Camera {
	theta := vfov * float32(math.Pi) / float32(180)
	half_height := Tanf(theta / 2)
	half_width := aspect * half_height
	w := vsub(lookfrom, lookat).unit_vector()
	u := vcross(vup, w).unit_vector()
	v := vcross(w, u)
	return Camera{
		origin: lookfrom,
//    248 <------><------>lower_left_corner = lookfrom - half_width * u - half_height * v - w,
		lower_left_corner: vsub(lookfrom, vadd(vmul(half_width, u), vadd(vmul(half_height, v), w))),
		horizontal: vmul(2 * half_width, u),
		vertical: vmul(2 * half_height, v),
	}
}
...
func main() {
	...
	cam := make_camera(Vec3{-2, 2, 1}, Vec3{0,0,-1}, Vec3{0,1,0}, 90, float32(nx) / float32(ny));
//	fmt.Print("cam=",cam,"\n")
	for j := ny-1; j >= 0; j-- {
		for i := 0; i < nx; i++ {
			col := Vec3{0, 0, 0}

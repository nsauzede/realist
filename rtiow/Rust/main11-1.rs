...
fn main() {
	...
	println!("{} {}", nx, ny);
	println!("255");
	let r = f32::cos(std::f32::consts::PI / 4 as f32);
	let
//	mut
	world: Vec<Box<dyn Hittable>> = vec!(
		Box::new(Sphere {center: Vec3([-r, 0., -1.]), radius: r, material: Box::new(Lambertian {albedo: Vec3([0., 0., 1.]) })}),
		Box::new(Sphere {center: Vec3([r, 0., -1.]), radius: r, material: Box::new(Lambertian {albedo: Vec3([1., 0., 0.])})}),
	);

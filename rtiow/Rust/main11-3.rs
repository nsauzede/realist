...
fn make_camera(lookfrom: Vec3, lookat: Vec3, vup: Vec3, vfov: f32, aspect: f32) -> Camera {
...
//	mut
	world: Vec<Box<dyn Hittable>> = vec!(
		...
		Box::new(Sphere {center: Vec3([-1., 0., -1.]), radius: -0.45, material: Box::new(Dielectric {ref_idx: 1.5})}),
	);

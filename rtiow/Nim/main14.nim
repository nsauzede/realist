        ...
        for b in -11..<11:
            ...
            if length(center - vec3(4, 0.2, 0)) > 0.9:
                if choose_mat < 0.8: # diffuse
                    ...
                    var r2 = 0.5f * (1f + random_f())
                    var r3 = 0.5f * (1f + random_f())
                    var r4 = 0.5f * random_f()
...
var arg = 1
if arg <= argc:
    nx = paramStr(arg).parseInt()
    arg+=1
    if arg <= argc:
        ny = paramStr(arg).parseInt()
        arg+=1
        if arg <= argc:
...
var aperture = 0f
var cam = make_camera(lookfrom, lookat, vec3(0, 1, 0), 30, float32(nx) /

...
struct hittable_s;
typedef bool (*hit_t)(struct hittable_s *p, const ray *r, float t_min,
...
bool list_hit(hittable *p, const ray *r, float t_min, float t_max,
              hit_record *rec) {
  hit_record temp_rec;
...
bool sphere_hit(hittable *p, const ray *r, float t_min, float t_max,
                hit_record *rec) {
...
#define HSPHERE(cx, cy, cz, r)                                                 \
  ((hittable){sphere_hit, .obj.sphere = (sphere_t){{cx, cy, cz}, r}})
#define HSTART                                                                 \
  { list_hit, .obj.null = 0 }
#define HEND                                                                   \
  { 0, .obj.null = 0 }
...
int main(int argc, char *argv[]) {
  ...
  printf("%d %d\n", nx, ny);
  printf("255\n");
  hittable world[] = {HSTART, HSPHERE(0, 0, -1, 0.5),
                      HSPHERE(0, -100.5, -1, 100), HEND};

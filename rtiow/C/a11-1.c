...
void make_camera(camera *cam, float vfov, float aspect) {
	...
	vcopy(cam->horizontal, VEC3(2*half_width, 0.0, 0.0));
	vcopy(cam->vertical, VEC3(0.0, 2*half_height, 0.0));
...
int main() {
	...
	hittable_t world[] = {
		...
		HSPHERE(R, 0, -1, R, MLAMBERTIAN(1, 0, 0)),
		HEND
	};
	camera cam;
	make_camera(&cam, 90, (float)nx/(float)ny);
